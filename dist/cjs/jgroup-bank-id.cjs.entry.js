...
const index = require('./index-abe2159a.js');
...
const StartButton = ({ onClick, isLoading, text, isOutlined, darkTheme, }) => {
    ...
    return (index.h("button", { type: "button", disabled: isLoading, onClick: onClick, class: {
        ...
        !isOutlined ? (index.h("img", { src: index.getAssetPath(darkTheme
                ? '/assets/bankid-logo-v2023.svg'
                : '/assets/bankid-logo-v2023-neg.svg'), class: "bankid-logo" })) : (''),
        text)) : (index.h("img", { src: index.getAssetPath((!isOutlined && !darkTheme) || (isOutlined && darkTheme)
            ? '/assets/spinner-white.svg'
            : '/assets/spinner-dark.svg') }))));
};
const CancelButton = ({ onClick, text, isLoading }) => (index.h("button", { type: "button", onClick: onClick, disabled: isLoading, class: "inline-flex items-center justify-center rounded-md bg-white w-20 h-9 px-3.5 py-2 text-sm font-semibold text-primary-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed" }, isLoading ? (index.h("img", { src: index.getAssetPath('/assets/spinner-dark.svg'), class: "h-5 w-5" })) : (`${text}`)));
const Alert = ({ message, type, onTryAgainButtonClick, tryAgainButtonText, }) => (index.h("div", { class: {
        ...
        index.h("div", { class: "ml-3" },
            ...
            type === 'error' ? (index.h("div", { class: "mt-4" },
                index.h("div", { class: "-mx-2 -my-1.5 flex" },
                    index.h("button", { type: "button", onClick: onTryAgainButtonClick, class: "rounded-md bg-red-50 px-3.5 py-2 text-sm font-semibold text-red-800 shadow-sm ring-1 ring-inset ring-red-300 hover:bg-red-100" }, tryAgainButtonText)))) : ('')))));
